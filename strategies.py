...
class SubStrat:
    ...
    def shift_and_resample(self):
        ...
        return self.data.get_column(self.forecast_col).fill_null(0).ewm_mean(span=6)
...
class VolatilityBands(SubStrat):
    ...
    def calc_forecast(self):
        ...
        if self.ma_type == 'sma':
            ...
            self.data = self.data.with_columns(
                pl.col('close').ewm_mean(span=self.lb).alias('ma')
            )
        ...
        self.data = self.data.with_columns(
            pl.col('z_score').ewm_mean(span=float(self.fast_ma)).alias('fast'),
            pl.col('z_score').ewm_mean(span=float(self.slow_ma)).alias('slow')
        )
