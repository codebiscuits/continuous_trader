...
# i write and test the code on my laptop, and run it live on a raspberry pi, so i have an empty file on the pi which i
# can check for to identify which machine and therefore whether to actually trade or just simulate.
in_production = Path("/pi_3.txt").exists()

# initialise the trader object
...
# run fresh backtests to calibrate the model with updated data
